...
`fig-width` changes the width of the figure. Larger values make the plot wider and smaller values make it narrower. 
...
The graph is misleading as percentages do not match the bar sizes. 
...
1858 is the total sample size. Service and response would be needed, and an ID variable would be helpful.
...
After loading the survation.csv file it was found that the percentage calculated matches the plot. 
...
survation_long |>
  ...
  arrange(factor(service, levels = c("Rail", "Water", "Energy", "Royal Mail")))
```
...
```{r}
#Assisted by ChatGPT
survation <- read_csv("data/survation.csv", show_col_types = FALSE)

service_levels <- c("Rail", "Water", "Energy", "Royal Mail")
response_levels <- c("Public sector", "Private sector", "Don't know")
...
survation_long <- survation |>
  ...
  mutate(
    service = factor(service, levels = rev(service_levels)),
    response = factor(response, levels = response_levels)
  )

survation_plot <- survation_long |>
  count(service, response) |>
  group_by(service) |>
  mutate(percent = n / sum(n) * 100)

ggplot(survation_plot, aes(x = percent, y = service, fill = response)) +
  geom_col() +
  scale_fill_manual(
...
The improved graph looks better than the original one as it matches the bar lengths with percentages and the services are in correct order. The colors also match the categories correctly. This graph is much clearer than the original one.
::: callout-tip
...
datasaurus_dozen |>
  ...
  print(n = 13)
```
For all the 13 datasets the summary statistics are almost similar. “mean x” and “mean y” are almost same. The same with standard deviation and correlation as well which shows that the datasets look similar. 
...
datasaurus_dozen |>
  ...
  facet_wrap(~ dataset)
```
Each dataset forms its own shape or its own pattern therefore the scatter plots are different from each other. In comparison with Q6, we can see that only numbers can’t tell us the whole story hence graphs are important to understand data as well. 
